        ...
        <div className="nt-main">
          ...
          <div className="nt-toolbar">
            <div className="nt-tools">
              ...
              <div ref={filterRef} style={{ position:"relative" }}>
                ...
                {filterOpen && (
                  <div className="nt-dropdown">
                    <div className="nt-dropdown-item" onClick={()=>{ selectAll(); setFilter("All"); setFilterOpen(false); }}>{t("notifications.filterAll")}</div>
                    <div className={`nt-dropdown-item${filter==="Read"?" active":""}`} onClick={()=>{ setFilter("Read"); clearSelection(); setFilterOpen(false); }}>{t("notifications.filterRead")}</div>
                    <div className={`nt-dropdown-item${filter==="Unread"?" active":""}`} onClick={()=>{ setFilter("Unread"); clearSelection(); setFilterOpen(false); }}>{t("notifications.filterUnread")}</div>
                  </div>
                )}
              </div>
              ...
              <button className="nt-tool-btn" onClick={handleMarkAllRead} title={t("notifications.markAllRead")} style={{ fontSize:10, padding:"0 6px", width:"auto" }}>{t("notifications.markAllRead")}</button>
            ...
            <div className="nt-pager">
              {t("notifications.showing", { from: pageStart+1, to: Math.min(pageStart+pageSize, filtered.length), total: filtered.length })}
          ...
          <div className="nt-list">
            {pageItems.length === 0
              ? <div style={{ padding:60, textAlign:"center", color:"#9CA3AF", fontSize:13 }}>{t("notifications.noNotifications")}</div>
              : pageItems.map(n => (
